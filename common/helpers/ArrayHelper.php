...
class ArrayHelper extends BaseArrayHelper
{
    ...
    // 补全顺序
    public static function itemsNumber($data, $page=1, $pageSize=10){
        if (!$data['total']) {
            return $data;
        }

        $nowPage = $data['page'] ?? $page;
        $nowPageSize = $data['pageSize'] ?? $pageSize;

        $startNuber = $data['total'] - (($nowPage - 1) * $nowPageSize);

        foreach($data['list'] as $k => $v) {
            $data['list'][$k]['now_number'] = $startNuber;
            $startNuber--;
        }
